...
import { getTopFourthUsers } from "@/features/top-users/top-users-slice";
import { serverUrl } from "@/helpers/shared";
import { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";

export default function FourthCoursTopUsers() {
 const dispatch = useDispatch();
 const { topFourthUsers } = useSelector((state: RootState) => state.topUsers);
    
  useEffect(() => {
    (async function () {
      try {
        if(!topFourthUsers?.length) {
            const res = await apiClient.get(
              "/statistics/top-Fourth-course-users"
            );
            dispatch(getTopFourthUsers(res.data));
        }
      } catch (error) {
        console.log(error);
      }
